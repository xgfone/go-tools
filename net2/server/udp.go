...
func UDPWithError(conn *net.UDPConn, handler UHandle, buf []byte, addr *net.UDPAddr) {
	...
	if result := handler.Handle(buf, addr); len(result) != 0 {
		if n, err := conn.WriteToUDP(result, addr); err != nil {
			log2.ErrorF("Failed to send the data to %s: %v", addr, err)
		} else {
			log2.DebugF("Send %v bytes successfully", n)
		}
	}
}
...
func UDPServerForever(addr string, size int, handle interface{}) error {
	...
	defer conn.Close()
	log2.DebugF("Listening on %v", addr)
	...
	for {
		...
		if err != nil {
			log2.ErrorF("Failed to read the UDP data: %v", err)
		} else {
...
func DialUDP(host, port interface{}) (*net.UDPConn, error) {
	addr := net2.JoinHostPort(host, port)
